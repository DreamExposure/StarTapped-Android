...
public class UnfollowBlogTask extends AsyncTask<Object, Void, NetworkCallStatus> {
    private TaskCallback callback;
    private final UUID blogId;

    public UnfollowBlogTask(TaskCallback _callback, UUID _blogId) {
        callback = _callback;
        blogId = _blogId;
    }
    ...
    protected NetworkCallStatus doInBackground(Object... objects) {
        ...
        try {
            ...
            if (response.code() == 200) {
                //Success
                return new NetworkCallStatus(true, TaskType.FOLLOW_UNFOLLOW_BLOG)
                ...
                return new NetworkCallStatus(false, TaskType.FOLLOW_UNFOLLOW_BLOG)
                        .setCode(response.code())
                        .setMessage(responseBody.getString("message"))
                        .setBody(responseBody);
            }
